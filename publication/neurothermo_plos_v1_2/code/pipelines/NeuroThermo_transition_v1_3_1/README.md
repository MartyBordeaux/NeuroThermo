...
## v1.3.1 figure hotfix

Version 1.3.1 changes no numerical decomposition result. It fixes the three
`01_boundary_*.png` plots: v1.3.0 accidentally used `DataFrame.mode` instead of
the `mode` column when filtering plotting data, producing empty boundary panels.
The corrected figures also show the fraction of core-secure biological pairs for
which at least 50% of within-pair parameter support reaches each boundary.
...
```bash
cd ~/neurothermo/v4
unzip neurothermo_transition_ensemble_v1.3.1.zip
