...
def make_figures(results_dir):
    ...
    ensemble = pd.read_csv(results_dir / "ensemble_convergence_summary.csv")
    scenario = pd.read_csv(results_dir / "scenario_convergence_gates.csv")
    cells = pd.read_csv(results_dir / "cell_balanced_deltas.csv")
    ...
    for (view, order), group in main.groupby(["view", "aggregation_order"]):
        ...
        axes[0].plot(x, group.median_pair_delta, marker="o", color=colors[view],
                     linestyle=styles[order], label="%s, %s" % (view, order.replace("_", " ")))
    ...
    axes[1].axhline(.80, color="black", linestyle="--", linewidth=.8)
    axes[1].set(ylim=(0, 1), ylabel="Scenario convergence pass fraction", xlabel="Distributional view")
    fine = min(cells.dt_ms)
    selected = cells[(cells.dt_ms == fine) & (cells.marker_variant == variant) &
                     (cells.aggregation_order == "marker_first")]
    arrays = [selected.loc[selected.view == view, "kl_minus_firing_p"].to_numpy() for view in ("xyz", "xy")]
    axes[2].boxplot(arrays, labels=["xyz", "xy"], showfliers=False)
    ...
    for label, axis in zip("ABC", axes):
        axis.text(-.13, 1.04, label, transform=axis.transAxes, fontweight="bold", fontsize=12)
    png = results_dir / "Fig_KL_convergence.png"
    pdf = results_dir / "Fig_KL_convergence.pdf"
